...
int main(int argc, char** argv)
{
    ...
    if (my_mpi_rank == 0 && g_opts.mmap_filename) {
        g_buffer = custom_mmap_alloc(g_opts.mmap_filename, g_opts.g_buffer_size,
                                     my_mpi_rank);
    } else {
        g_buffer = NULL;
    ...
    if (my_mpi_rank == 0) {
        ...
        ret = margo_bulk_create(mid, 1, &buffer, &g_opts.g_buffer_size,
...
static int parse_args(int argc, char** argv, struct options* opts)
{
    ...
    while ((opt = getopt(argc, argv, "n:x:c:T:t:D:m:X:aw:")) != -1) {
        switch (opt) {
        case 'x':
